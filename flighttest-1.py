...
from airsimneurips import MultirotorClient, Vector3r
...
# Extract coordinates and fit splines
x_vals = [pos.x_val for pos in full_path]
y_vals = [pos.y_val for pos in full_path]
z_vals = [pos.z_val for pos in full_path]
t_vals = list(range(len(full_path)))

spline_x = CubicSpline(t_vals, x_vals)
spline_y = CubicSpline(t_vals, y_vals)
spline_z = CubicSpline(t_vals, z_vals)

# Fewer samples for faster traversal
samples = np.linspace(0, len(full_path) - 1, num=len(full_path) * 3)
spline_points = [Vector3r(float(spline_x(t)), float(spline_y(t)), float(spline_z(t))) for t in samples]

# Move quickly along the spline
client.moveOnSplineAsync(spline_points, vel_max=12.0, acc_max=6.0, vehicle_name="drone_1").join()
